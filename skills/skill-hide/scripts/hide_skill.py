...
from config_manager import get_skill_hide_self_path, normalize_path, read_config, add_to_history


def hide_skills(skill_names, folders=None, specific_folder=None):
    """
    隐藏技能（将SKILL.md重命名为SKILL.md.hide）
    ...
    for skill_name in skill_names:
        ...
        # 检查是否试图隐藏自己
        if skill_name == "skill-hide":
            result["skipped"].append({"name": skill_name, "reason": "cannot hide skill-hide itself"})
            continue

        for folder in search_folders:
            skill_path = os.path.join(folder, skill_name)
            skill_md_path = os.path.join(skill_path, "SKILL.md")
            skill_hide_path = os.path.join(skill_path, "SKILL.md.hide")

            # 检查是否已经被隐藏（SKILL.md.hide 存在但 SKILL.md 不存在）
            if os.path.exists(skill_hide_path):
                found_count += 1
                result["skipped"].append({"name": skill_name, "path": skill_path, "reason": "already hidden"})
                continue  # 继续检查其他文件夹
            ...
            if os.path.exists(skill_md_path):
                ...
                try:
                    # 执行隐藏操作
                    os.rename(skill_md_path, skill_hide_path)
        ...
        if found_count == 0:
            result["failed"].append({"name": skill_name, "reason": "skill not found in configured folders"})

    return result


def get_all_skills(folder):
    """
    获取指定文件夹下的所有技能列表
    ...
    for item in os.listdir(folder):
        ...
        if item.startswith('.') or item == 'skill-hide':
            continue
        # 检查是否是技能文件夹（包含SKILL.md或SKILL.md.hide）
        if os.path.isdir(item_path):
            if os.path.exists(os.path.join(item_path, "SKILL.md")) or \
               os.path.exists(os.path.join(item_path, "SKILL.md.hide")):
                skills.append(item)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='隐藏技能')
    parser.add_argument('skills', nargs='*', help='要隐藏的技能名称列表')
    parser.add_argument('--folder', help='指定在哪个父文件夹下操作')
    parser.add_argument('--all', action='store_true', help='隐藏指定文件夹下的所有技能')
    ...
    if args.all:
        if args.folder:
            ...
            if not os.path.exists(target_folder):
                print(f"错误: 指定的文件夹不存在: {target_folder}")
                sys.exit(1)
            all_skills = get_all_skills(target_folder)
            if not all_skills:
                print(f"在 {target_folder} 下没有找到可隐藏的技能")
                sys.exit(0)
            print(f"在 {target_folder} 发现 {len(all_skills)} 个技能: {', '.join(all_skills)}\n")
                    ...
                    for item in os.listdir(folder):
                        ...
                        if os.path.isdir(item_path):
                            if os.path.exists(os.path.join(item_path, "SKILL.md")):
                                all_visible.append({"name": item, "path": item_path, "folder": folder})
            if not all_visible:
                print("在所有配置文件夹中都没有找到可隐藏的技能")
                sys.exit(0)
            print(f"发现 {len(all_visible)} 个可隐藏技能:\n")
            for skill in all_visible:
                print(f"  - {skill['name']} ({skill['path']})")
            print()
            # 直接逐个隐藏，不调用 hide_skills
            result = {"success": [], "failed": [], "skipped": []}
            for skill in all_visible:
                if skill['name'] == "skill-hide":
                    result["skipped"].append({"name": skill['name'], "path": skill['path'], "reason": "cannot hide skill-hide itself"})
                    continue
                skill_md_path = os.path.join(skill['path'], "SKILL.md")
                skill_hide_path = os.path.join(skill['path'], "SKILL.md.hide")
                try:
                    if os.path.exists(skill_hide_path):
                        result["skipped"].append({"name": skill['name'], "path": skill['path'], "reason": "already hidden"})
                    else:
                        os.rename(skill_md_path, skill_hide_path)
        ...
        result = hide_skills(args.skills, target_folders, args.folder)
    ...
    if result["success"]:
        ...
        if history_folder:
            add_to_history(history_folder, 'hide')
