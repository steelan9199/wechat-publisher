...
3. 创建数据表（使用 [table/create-single.js](../scripts/table/create-single.js)）
4. 添加记录（使用 [record/create.js](../scripts/record/create.js) 或 [record/batch-create.js](../scripts/record/batch-create.js)）

### 场景2：查询记录并更新

1. 获取访问凭证 `tenant_access_token`
2. 解析多维表格 URL 获取 `app_token` 和 `table_id`
3. 查询记录（使用 [record/get.js](../scripts/record/get.js)）
4. 更新记录（使用 [record/update.js](../scripts/record/update.js)）

### 场景3：上传附件并关联到记录

1. 获取访问凭证 `tenant_access_token`
2. 上传素材（使用 [media/upload.js](../scripts/media/upload.js)）获取 `file_token`
...
| 查询无结果 | table_id 或 view_id 错误 | 确认从 URL 解析的 ID 正确，使用 [parse_bitable_url.md](parse_bitable_url.md) 工具验证 |
| 附件上传失败 | 文件过大或格式不支持 | 检查文件大小限制（通常 20MB），确认文件格式符合要求 |
| 权限错误 | 应用未添加到表格 | 按照 [authentication.md](authentication.md) 中的步骤将应用添加到多维表格 |
