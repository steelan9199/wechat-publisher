...
- **v4.0** (2026-05-08): 🔥 自动编号管理
  - ✨ 新增：**自动编号功能**，扫描已有备份文件夹，按顺序生成下一个编号
  - ✨ 新增：支持 `require("config.js")` 不带 `./` 前缀的格式
  - ✨ 新增：用户只需提供描述性名称，脚本自动生成完整名称（如"07测试配色"）
  - 💡 优化：无需用户手动管理两位数字编号，专注编写代码

- **v3.0** (2026-05-08): 🔥 Node.js 重构
  - ✨ 新增：使用 Node.js v24.15.0 ES Modules 重写备份脚本
  - ✨ 新增：`.mjs` 原生模块支持，无需 transpile
  - ✨ 新增：`path.relative()` 路径计算，跨平台兼容
  - ✨ 新增：`Set` 替代 `HashSet`，更简洁的循环依赖检测
  - ✨ 新增：`matchAll()` 正则全局匹配，更高效的依赖提取
  - 🐛 修复：PowerShell 在 Windows 中文路径下的编码问题
  - 🐛 修复：PowerShell `Copy-Item` 在长路径下的限制
  - ♻️ 重构：完全移除 PowerShell 依赖，统一使用 Node.js

- **v2.0** (2026-05-08): 🔥 重大升级 - 递归依赖检测
  - ✨ 新增：**递归嵌套依赖检测算法（BFS）**
  - ✨ 新增：**依赖树可视化输出**
  - ✨ 新增：**完整的PowerShell一键备份脚本**
  - 🐛 修复：v1.0只检测直接依赖，会遗漏嵌套依赖的问题
