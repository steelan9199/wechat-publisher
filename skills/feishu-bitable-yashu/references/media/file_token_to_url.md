...
1. **创建参数配置文件**
   ...
   ```json
   {
     ...
     "extra": "{\"bitablePerm\":{\"tableId\":\"your_table_id\"}}"
   }
   ```
...
2. **运行脚本**
   ...
   ```bash
   # bash/zsh
   cd $SKILL_DIR/scripts && node media/file-token-to-url.js --parameter-file-path "$SKILL_DIR/temp/feishu-operation-1740374400000-a7x9k2.json"

   # PowerShell 5
   cd $SKILL_DIR/scripts; if ($?) { node media/file-token-to-url.js --parameter-file-path "$SKILL_DIR/temp/feishu-operation-1740374400000-a7x9k2.json" }
   ```
