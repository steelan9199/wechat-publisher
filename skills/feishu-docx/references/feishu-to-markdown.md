...
```bash
node scripts/feishu-to-markdown.js --parameter-file-path <参数文件绝对路径>
```
...
| output_dir          | string  | no   | 输出目录路径         | 当前技能`feishu-docx`目录下的「飞书云文档下载」文件夹 |
| download_images     | boolean | no   | 是否下载图片         | true                                     |

#### output_dir 参数补充说明

- 如果不提供该参数，会自动在当前工作目录下创建 `飞书云文档下载` 文件夹作为根输出目录
- 会自动在根输出目录下创建**以文档标题命名的子文件夹**，所有下载内容都保存在该子文件夹中
...
```json
{
  ...
  "data": {
    "document_id": "doxcnePuYufKa49ISjhD8Iabcef",
    "document_title": "文章标题",
    "markdown_file_path": "D:/output/my-article/文章标题.md",
    "blocks_count": 25,
    "images_downloaded": 3
  }
}
...
| 10003   | invalid param          | 检查参数格式是否正确         |
