...
本主题的 golden source 是当前技能内的 `assets/template-swiss.html` 与本文档登记的 `S01-S22`。生成正文页时不要把 Swiss 当成“自由组合的风格包”。默认只能使用本文档登记的 `S01-S22`。每个 slide 都必须在 `<section>` 上写 `data-layout="Sxx"`。

**关键约束**:

- 顶部中文标题默认左对齐并处在左上内容轴;不要把标题放到页面中间。
- 不允许临时发明原始 22P 之外的正文结构。
- 需要单张大图时使用 `S22 Image Hero`;需要多图时用 `S15/S16` 的原始矩阵/小报骨架改造成图片格。
- 地点、路线、人物住所、城市关系页优先用 S08 的右侧插槽做几何关系图;不得依赖外部组件文件。
- SVG 只画几何,不写可见文字。标签放 HTML 里。
- 生成完成后直接检查每页是否包含 `data-layout="Sxx"`、主题类是否来自模板、内容是否避开底部 nav 安全区。
...
- 12 列 grid:`grid-template-columns:repeat(12,1fr)` + `gap:16px`
...
<div class="canvas-card">
  <div class="chrome-min">...</div>
  <div
    style="flex:1;padding:0;display:grid;grid-template-rows:auto 1fr auto;gap:3vh"
  >
    主体内容
  </div>
</div>
...
<div
  data-anim="head"
  style="display:grid;grid-template-columns:auto 1fr;gap:3vw;align-items:end"
>
...
- 使用图片底对齐组件时,模板会自动给底部加安全区,避免图片 caption 被分页组件挡住
...
**6. 网格/卡片垂直空间预算** ⚠️ 防裁切
`.canvas-card` 固定 100vh,`.slide` 有 `overflow:hidden`。当使用 `.grid-4` / `.grid-6` / `.four-cards` / `.cell-6` 等网格时,标题 + 卡片总高度可能超出容器导致底部裁切。

模板已默认去掉了 `.grid-4` / `.grid-6` 的 `flex:1`,改为 `grid-template-rows:auto`,让网格自适应内容高度。但内容过多时仍需手动收紧:

| 情况                    | 修复策略(按优先级)                                |
| ----------------------- | ------------------------------------------------- |
| stat-note 长文本(>15字) | ① 缩小 stat-nb 字号 ② 降低 stat-note 字号 + 行高  |
| 标题 + 引语 + 6 卡      | ③ 缩紧 grid gap ④ 减小 lead 字号 ⑤ 降低 h-xl 字号 |
| 标题 + 4 卡 + 长段落    | ⑥ 缩紧 four-cards gap ⑦ 段落改用 `body-sm`        |

**不要**用 `overflow:visible` 绕过裁切——文字会溢出到 canvas-card 边界外。
...
- 只有用户明确提供本地图片/截图/图表素材时,才使用图片版式;不要主动调用 AI 生图,不要默认外链图库
- 使用 2-3 张用户提供素材时,必须先绑定原始版式槽位:单张大图 = S22;多图 = S15/S16 网格改造
- S22 的照片主体必须位于中央安全区,HTML 用 `object-position:center 35%` 或 `center center`,不要用 `top center` 截人脸
- 没有真实图片素材时,改用 P3/P10/P12/P19 等纯文字、几何、卡片版式
...
1. 同时打开当前 `template-swiss.html` 或生成页、正在修改的测试 PPT。
2. 截图前先等入场动效稳定(约 1-2 秒)。不要把动画中间态误判成"内容缺失"或"版式空白"。
3. 先看视觉:标题重量、头部距离、图片落位、底部安全区、caption 是否被 nav 挡住。
4. 对照本文档登记的同类版式,不要只对照 CSS helper;以实际页面结构和视觉结果为准。
5. 再回到代码,检查该页是否误用了不属于该版式的组件,例如把 P7 图表用于没有真实数值的概念列表。
...
<section class="slide accent" data-animate="hero">
  <div class="canvas-card">
    ...
    <div
      style="flex:1;padding:0;display:grid;grid-template-rows:auto 1fr auto;gap:2.6vh"
    >
      <div
        data-anim="kicker"
        class="t-meta"
        style="color:rgba(255,255,255,.78);letter-spacing:.22em"
      >
        [必填] 章节英文 / Section En
      </div>
      <h1
        data-anim="title"
        style="align-self:center;font-family:var(--sans),var(--sans-zh);font-weight:200;font-size:min(11.6vw,19vh);line-height:.94;letter-spacing:-.025em;color:#fff"
      >
        [必填] 中文主标题<br />(可在某字加
        <span style="font-style:italic;font-weight:300">italic</span> 微强调)
      </h1>
      <div
        data-anim="bottom"
        style="display:grid;grid-template-rows:auto auto;gap:1.6vh;border-top:1px solid rgba(255,255,255,.22);padding-top:2vh"
      >
        <div
          data-anim="lead"
          class="lead"
          style="max-width:52ch;color:rgba(255,255,255,.86);font-weight:300"
        >
          [必填] 一段 1-2 行的副标 / 引子,定调全场.
        </div>
        <div style="display:flex;justify-content:space-between;align-items:end">
          <div class="t-meta" style="color:rgba(255,255,255,.6)">
            [选填] 作者 · 日期 · 出处
          </div>
          <div class="t-meta" style="color:rgba(255,255,255,.6)">
            → swipe / arrow keys
...
**垂直空间**:6 格内容中 `.canvas-card` 高度固定(100vh),确保所有 cell 的图标+编号+标题+描述总高度不溢出。若底部被裁,优先:

1. 缩紧 `.cell-6` 的 grid gap
2. 减小描述字号与行高
   **不要**用 `overflow:visible` 绕过。  
   **示例代码**:
...
<section class="slide split" data-animate="split-statement">
  <div class="canvas-card">
    <div class="split-half">
      ...
      <div
        class="half b-accent"
        style="padding:5.6vh 3.6vw 4.4vh;justify-content:space-between;position:relative;overflow:hidden"
      >
        <canvas class="ascii-bg" aria-hidden="true"></canvas>
        <div
          class="chrome-min"
          style="margin-bottom:0;position:relative;z-index:1"
        >
        ...
        <div
          data-anim="manifesto"
          style="display:flex;flex-direction:column;gap:2vh;position:relative;z-index:1"
        >
          <div
            class="t-meta"
            style="color:rgba(255,255,255,.78);letter-spacing:.22em;margin-bottom:1.6vh"
          >
            MANIFESTO
          </div>
          <h2
            style="font-family:var(--sans),var(--sans-zh);font-size:min(8vw,14vh);line-height:.94;letter-spacing:-.025em;font-weight:200;color:#fff"
          >
            [必填] Build a model.<br />Run
            <span style="font-style:italic;font-weight:300">forever</span>.
          </h2>
          <div
            style="font-family:var(--sans),var(--sans-zh);font-size:max(13px,1vw);line-height:1.6;color:rgba(255,255,255,.82);font-weight:300;max-width:36ch;margin-top:1.4vh"
          >
            [必填] 一句中英文落地注脚.
          </div>
        </div>
        <div
          data-anim="signature"
          style="display:flex;justify-content:space-between;align-items:end;border-top:1px solid rgba(255,255,255,.22);padding-top:2vh;position:relative;z-index:1"
        >
          <div class="t-meta" style="color:rgba(255,255,255,.62)">
            [选填] 作者 · 头衔
          </div>
      ...
      <div
        class="half"
        style="padding:5.6vh 3.6vw 4.4vh;justify-content:space-between"
      >
        <div class="chrome-min">
        ...
        <div class="t-meta" style="color:var(--text-helper);text-align:right">
          → 完 · END OF FIELD NOTE
        </div>
      </div>
    </div>
...
<div class="canvas-card">
  <span
    class="ring-mat"
    style="left:5vw;bottom:5vh;width:18vw;height:18vw"
  ></span>
  <h1 class="h-statement">
    Build a thin harness.<br />Write fat skills.<br />Codify everything.
  </h1>
...
**垂直空间**:4 列卡中内容总高(顶线 + 标题 + 4×(t-meta + 大字标题 + 段落))必须能全部容纳在 `.canvas-card` 内。若底部文字被裁,优先:

1. 缩紧 `.four-cards` 的 gap
2. 减小段落字号(用 `body-sm` 替代默认 `body`)
3. 减小标题字号
   **不要**用 `overflow:visible` 绕过。
...
- 图片只使用用户提供的本地文件,例如 `images/{页号}-{语义}.png`;不要调用 AI 生图,不要默认外链 unsplash
...
<section class="slide" data-layout="S22" data-animate="image-hero">
  ...
  >
    <div
      data-anim="img"
      style="position:relative;flex:0 0 60%;overflow:hidden;background:var(--grey-1)"
    >
      <img
        src="images/22-product-scene.png"
        alt="[必填] 图片说明"
        loading="eager"
      ...
      <div
        class="chrome-min"
        style="position:absolute;top:0;left:0;right:0;color:rgba(255,255,255,.9);padding:5.6vh 5vw 0"
      >
      ...
      <div
        ...
        style="position:absolute;left:5vw;top:11vh;background:var(--paper);padding:3.2vh 3.2vw;max-width:40vw"
      >
        <div
          style="font-family:var(--sans),var(--sans-zh);font-weight:200;font-size:min(5.2vw,9vh);line-height:1;letter-spacing:-.035em;color:var(--text-primary)"
        >
    ...
    <div data-anim="kpi" class="image-hero-body">
      <div
        style="max-width:48ch;font-family:var(--sans),var(--sans-zh);font-size:max(15px,1.3vw);line-height:1.55;font-weight:300;color:var(--text-primary);letter-spacing:-.005em"
      >
      ...
      <div class="image-hero-stats" style="gap:4vw">
        <div style="display:flex;flex-direction:column;gap:.6vh">
          ...
          <div
            style="font-family:var(--sans);font-weight:200;font-size:min(4.6vw,7.6vh);line-height:.95;letter-spacing:-.04em"
          >
            12×
          </div>
          <div
            style="height:1px;background:var(--border-subtle);margin-top:auto"
          ></div>
          <p class="body-sm">[必填] 指标解释</p>
        </div>
        <div style="display:flex;flex-direction:column;gap:.6vh">
          <div style="height:1px;background:var(--ink)"></div>
          <div class="t-meta">Metric 02</div>
          <div
            style="font-family:var(--sans);font-weight:200;font-size:min(4.6vw,7.6vh);line-height:.95;letter-spacing:-.04em"
          >
            3.4h
        ...
        <div style="display:flex;flex-direction:column;gap:.6vh">
          <div style="height:1px;background:var(--ink)"></div>
          <div class="t-meta">Metric 03</div>
          <div
            style="font-family:var(--sans);font-weight:200;font-size:min(4.6vw,7.6vh);line-height:.95;letter-spacing:-.04em;color:var(--accent)"
          >
            100%
          </div>
          <div
...
| 地点 / 路线 / 人物住所关系 | P8 Duo Compare 的右侧插槽做几何关系图 |
| 单图解释论点 / 图文混排    | P22 Image Hero                        |
| 2-3 张图片/截图/图表证据链 | P15/S16 图片格改造                    |
...
| 地点/路线/城市关系          | P8 Duo Compare 的右侧插槽做几何关系图                | 普通 S04/S16 卡片罗列                            |
...
| 1 张核心图片 + 一段解释     | P22 Image Hero                                       | 无图源时不要硬做图片页                           |
| 2-3 张同类图片              | P15/S16 图片格改造                                   | P4/P16 纯文字卡片直接塞截图                      |
