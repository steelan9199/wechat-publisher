...
description: "生成扁平的 SVG 图（架构图/中心辐射图、线性流程图、简易时序图），也支持把已有 SVG 文件转成 PNG 图片。激活条件（满足任一即可）：生成类关键词 `画架构图`、`画流程图`、`画时序图`、`画个状态机`；转换类关键词 `SVG 转图片`、`把 SVG 转成图片`、`把 SVG 转成 PNG`。"
---
...
1. **生成模式：生成 SVG 图** —— 按你的描述画出扁平、随主题切换的架构图 / 流程图 / 时序图 / 状态机，并保存为 `.svg` + 一张白底 `.html` 预览图 + 一张 2× 的 `.png`（用于插入公众号等文章）。
2. **转换模式：把已有 SVG 转成图片** —— 你手里有一份 `.svg` 文件（本技能生成的，还是别处来的都行），技能调用自带转换器把它栅格化成 2× 白底 `.png`，同样方便插入公众号等场景。

绘图规范完全内置在本技能内（见 `references/design-system.md`），不依赖任何外部 AI 客户端命令。
