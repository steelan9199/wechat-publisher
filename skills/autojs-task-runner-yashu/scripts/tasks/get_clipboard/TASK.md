---
name: get_clipboard
description: "获取手机剪贴板文本内容：调用AutoJS全局getClip()读取剪贴板文本。短文本(≤100字)直接回传聊天查看；长文本(>100字)自动落地成文件到电脑 scripts/uploads/ 目录并只回传绝对路径+前80字预览（省token、不刷屏）。"
args: {}
---

# get_clipboard · 获取手机剪贴板内容
...
- 想拉的是文件（图片/文档/脚本） → 用 `download_file`，本模板只回传纯文本。
- 想要的是"看屏幕" → 用 `screenshot` / `crop_screenshot`。

## 参数细节与坑
- 本模板无必填参数，直接调用即可：`node run_task.js get_clipboard`。
- 返回字段：
...
- 手机端未运行常驻客户端（手机离线）：`run_task.js` 会返回 `503 / 手机未连接`，不是本模板的问题，先确认手机在线。

## 示例调用
```bash
# 获取当前剪贴板内容（最常见用法）
node run_task.js get_clipboard
