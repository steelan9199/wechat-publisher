 ...
 * beep 音频不要求用户手机里事先存在：放在 PC 下发数据目录
 * （scripts-from-computer/data/beep.wav），主流程启动时自动检测，不存在才现场生成
 * （正弦波 PCM 手工拼 WAV，移植自 make-beep 模板）。
 * ================================================================== */
var sdcardPath = files.getSdcardPath(); // /storage/emulated/0
var pcDataDir = files.join(sdcardPath, "脚本", "scripts-from-computer", "data"); // PC 下发数据目录
var beepFilePath = files.join(pcDataDir, "beep.wav"); // .../scripts-from-computer/data/beep.wav

// 确保 beep 提示音存在：已存在直接跳过，不存在才现场生成
function ensureBeepFile() {
  if (files.exists(beepFilePath)) {
    return true;
  }
  var sr = 8000; // 采样率(Hz)
  var dur = 0.3; // 时长(秒)
  ...
  for (var i = 0; i < n; i = i + 1) {
    var v = Math.sin((2 * Math.PI * freq * i) / sr) * 0.8 * 32767;
    if (v > 32767) {
      v = 32767;
    }
    if (v < -32767) {
  ...
  files.ensureDir(beepFilePath); // 只确保 脚本根目录(动态拼接)/ 目录链存在，不碰文件本身
  return files.writeBytes(beepFilePath, bos.toByteArray());
}
function playBeep(beepFilePath) {
  media.playMusic(beepFilePath);
  var duration = media.getMusicDuration();
  sleep(duration);
}
function alertErrorSound(beepFilePath) {
  var i;
  for (i = 0; i < 3; i = i + 1) {
    playBeep(beepFilePath);
    if (i < 9) {
      sleep(2000);
    } // 前 9 次播完后停顿 2 秒，第 10 次不再等待
  }
}
...
function isChrome(t) {
  ...
  if (t === "公众号" || t === "订阅号" || t === "订阅号消息") {
    return true;
  } // 标题本身
  if (/^[A-Za-z#]{1,3}$/.test(t)) {
    return true;
  } // 右侧 A-Z / # 索引字母（含 OCR 粘连成 2~3 字母的情况，如 EF/ABC）
  if (/^\d+个公众号$/.test(t)) {
    return true;
  } // 列表底部计数标签（如「8个公众号」「1个公众号」），不是账号——
  // 实测教训：当唯一账号是 OCR 盲区（整行识别不出）时，OCR 只能看到这个标签，
  // 旧版会把它当成账号去长按，永远弹不出菜单而卡死。
  return false;
}
...
function topHasOfficial(det, img) {
  ...
  for (i = 0; i < det.length; i = i + 1) {
    if (det[i].text && det[i].text.indexOf("公众号") >= 0) {
      if (det[i].bounds.top < img.height / 3) {
        return true;
      }
    }
  }
...
function titleBottom(det, img) {
  ...
  for (i = 0; i < det.length; i = i + 1) {
    if (det[i].text && det[i].text.indexOf("公众号") >= 0) {
      if (
        det[i].bounds.top < img.height / 3 &&
        (topMost < 0 || det[i].bounds.top < topMost)
      ) {
  ...
  for (i = 0; i < det.length; i = i + 1) {
    if (
      det[i].text &&
      det[i].text.indexOf("公众号") >= 0 &&
      det[i].bounds.top === topMost
    ) {
...
function pickFirstAccount(det, img) {
  ...
  for (i = 0; i < det.length; i = i + 1) {
    ...
    if (t.length < 2 || t.length > 40) {
      continue;
    } // 过滤索引字母/超长预览（上限放宽到 40，避免长名公众号如「Cherry Studio 全能AI工作站」被误杀）
    if (o.bounds.top <= tb) {
      continue;
    } // 必须在「公众号」标题下方
    if (o.bounds.right > img.width * 0.88) {
      continue;
    } // 排除最右侧字母索引条区域（几何拦截，不依赖 OCR 文字内容）
    if (best == null || o.bounds.top < best.bounds.top) {
      best = o;
    }
  }
  return best;
...
function findText(det, sub) {
  var i;
  for (i = 0; i < det.length; i = i + 1) {
    if (det[i].text && det[i].text.indexOf(sub) >= 0) {
      return det[i];
    }
  }
  return null;
...
// 解析列表底部计数标签「N个公众号」→ 返回 N（解析不到返回 -1）。
// 计数是 OCR 唯一稳定可读的「真值」：账号名可能整行不被识别（OCR 盲区，如生僻字），
// 但「N个公众号」实测每次都能读出——用它区分「列表真空」vs「有账号但 OCR 看不见」。
function parseAccountCount(det) {
  var i, m;
  for (i = 0; i < det.length; i = i + 1) {
    if (det[i].text) {
      m = det[i].text.match(/(\d+)个公众号/);
      if (m) {
...
function findUnfollowButton(det, img) {
  ...
  for (i = 0; i < det.length; i = i + 1) {
    o = det[i];
    if (o.text && o.text.indexOf("不再关注") >= 0) {
      all.push(o);
    }
  }
  if (all.length === 0) {
  ...
  if (left != null) {
    ...
    for (i = 0; i < all.length; i = i + 1) {
      ...
      if (rx > lx && Math.abs(ry - ly) <= rowH) {
        // 必须在右、且与左按钮同水平行
        if (Math.abs(ry - ly) < bestDy) {
          bestDy = Math.abs(ry - ly);
          best = o;
        }
      }
    }
...
try {
  ...
  // 确保 beep 提示音存在（不存在则现场生成）；失败只打日志，不影响取关主流程
  try {
    ensureBeepFile();
  } catch (eBeep) {
    console.log("[beep] 生成提示音失败（不影响取关流程）：" + eBeep);
  }

  // 截图权限前置：后台线程正则多候选自动点掉授权按钮（文案因 ROM 而异），否则 captureScreen 会卡死/返回空
  ...
  while (loopCount < MAX_LOOP) {
    ...
    if (best == null) {
      // OCR 一个账号都选不到：先读计数标签，分辨「列表真空」还是「OCR 盲区账号」
      var cnt = parseAccountCount(det);
      if (cnt <= 0) {
        // 列表已空 → 取关完成
        result = { ok: 1, msg: "列表已空，取关完成", unfollowed: done };
        console.log(
          "[名单校验] 通过✅ 公众号列表已空，取关完成，共 " + done + " 个",
        );
        break;
      }
      // 兜底：计数 > 0 但选不到账号 → 存在 OCR 盲区账号（整行识别不出，如生僻字名），
      // 按坐标长按「标题下缘 与 计数标签上缘 之间」的第一行，完全不依赖文字识别
      var cntItem = findText(det, "个公众号");
      var topY = titleBottom(det, img);
      var botY = cntItem != null ? cntItem.bounds.top : img.height * 0.8;
      if (botY <= topY + 40) {
        botY = topY + Math.round(img.height * 0.1);
      }
      cx = Math.round(img.width * 0.34); // 账号名文字列中心（头像右侧），避开右缘索引条
      cy = Math.round((topY + botY) / 2);
      targetName = "OCR盲区账号(剩" + cnt + "个)";
      console.log(
        "[兜底] 计数=" +
          cnt +
          " 但 OCR 选不到账号 → 按坐标长按第一行 (" +
          cx +
          "," +
          cy +
          ")",
    ...
    for (k = 0; k < 4; k = k + 1) {
      img = captureScreen();
      det = ocr.detect(img);
      if (hasText(det, "仍然关注") || hasText(det, "不再关注")) {
        page = "CONFIRM";
        break;
      }
      sleep(400);
    ...
    console.log(
      "[取关成功] 已取消关注【" + targetName + "】，累计 " + done + " 个",
    );
  }
  ...
  if (loopCount >= MAX_LOOP && result.ok == 1) {
    result = {
      ok: 0,
      err: "已达循环上限 " + MAX_LOOP + " 次，列表仍未清空，疑似异常，已停止",
      unfollowed: done,
    };
  }
} catch (e) {
