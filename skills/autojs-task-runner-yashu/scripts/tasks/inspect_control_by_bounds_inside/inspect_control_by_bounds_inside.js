/**
 * 模板名：inspect_control_by_bounds_inside
 * 用途：传入矩形 (left, top, right, bottom)，用 UiSelector.boundsInside() 匹配「包围盒【完全位于】该矩形内」的控件，
 *       返回最佳命中控件信息 + 可复用定位选择器。用于「给定一个区域，想知道里面有哪些控件、拿最具体那个的稳定选择器」。
 *       是 inspect_control 家族的「按区域(内含)」版本；与 by_bounds(精确矩形) 不同，这里是区域包容语义，通常匹配多个。
 * 参数（args，必填带 *，选填带 ?）：
 ...
 *   // returnAll:true——额外回 controls（≤10，count 仍为区域真实总数 35）
 *   { "ok": 1, "count": 35, "control": {...}, "selectors": {...},
 *     "controls": [ { "control": {...}, "selectors": {...} }, ... ] }
 * 说明：
 *   - boundsInside() 为「区域内含」匹配：控件矩形须完全落在给定矩形内，通常命中很多（count 为区域真实总数，可能几十上百）。
...
try {
  ...
  if (args.left === undefined || args.top === undefined ||
    ...
    var all = boundsInside(args.left, args.top, args.right, args.bottom).find();
    var hits = [];
