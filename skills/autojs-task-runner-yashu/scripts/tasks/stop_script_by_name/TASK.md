---
name: stop_script_by_name
description: "按脚本文件名停止手机端正在运行的 AutoJS 实例：调用 engines.all() 找出同名引擎并 forceStop，用于清理重复/卡死的后台脚本；默认保护当前运行引擎，绝不误停正在执行任务的自己。"
args: { "name": "string*", "waitMs": "number", "includeSelf": "boolean", "caseSensitive": "boolean" }
---

# stop_script_by_name · 按文件名停止手机端脚本实例
...
- 想对界面做点击/滑动/读控件：那是 `tap_text`/`swipe`/`inspect_control_*` 的职责，本模板不碰 UI，只管引擎进程。
...
node run_task.js --path tasks/stop_script_by_name/stop_script_by_name.js --args '{"name":"autojs-task-phone-client.js"}'

# 只按名停止、不等资源释放
node run_task.js --path tasks/stop_script_by_name/stop_script_by_name.js --args '{"name":"autojs-task-phone-client.js","waitMs":0}'
```
...
## 红线提醒
- 不做微信自动化（风控封号），本模板与微信无关，但调用方不得借它去折腾微信相关脚本的调度链路。
