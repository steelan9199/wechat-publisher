 ...
 *                         - 绝对路径（以 "/" 开头，如 "/sdcard/脚本/autojs_list_ui.js"）：直接定位该文件。
...
function findByName(name, roots) {
  ...
  // 再在 /sdcard/autojs 下递归（深度 4）
  var deep = deepFind("/sdcard/autojs", name, 4);
  if (deep) return deep;
  // 兜底：/sdcard 下递归（深度 3）
  return deepFind("/sdcard", name, 3);
}
