    ...
    new android.view.View.OnTouchListener((view, event) => {
      ...
      switch (event.getAction()) {
        ...
        case event.ACTION_MOVE:
          //移动距离过大则判断为移动状态
          if (
            Math.abs(event.getRawY() - this.y) > 5 &&
            Math.abs(event.getRawX() - this.x) > 5
          ) {
            //移动状态清除定时器
            if (this.Timeout) {
              //定时器存在则清除定时器。
            ...
            //移动手指时调整悬浮窗位置
            window.setPosition(
              this.windowX + (event.getRawX() - this.x),
              this.windowY + (event.getRawY() - this.y),
            );
          }
          return true;
        //抬起事件。
...
var inputBoxViewArr = [window.appName, window.邀请码];
...
window.停止指定app.on("click", function () {
  var appName = window.appName.getText();
  toastLog("您输入的APP是" + appName);
  停止指定app(appName);
  toastLog("停止指定app结束" + appName);
});
window.启动指定app.on("click", function () {
  var appName = window.appName.getText();
  toastLog("您输入的APP是" + appName);
  启动指定app(appName);
  toastLog("启动指定app结束" + appName);
});
window.停止最新app.on("click", function () {
  var appName = 停止最新app();
  toastLog("停止指定app结束" + appName);
});
window.启动最新app.on("click", function () {
  var appName = 启动最新app();
  toastLog("启动指定app结束" + appName);
});
// setTimeout(() => {
...
window.输入邀请码.on("click", function () {
  window.disableFocus();
  setTimeout(() => {
...
function 使输入框点击时输入法弹出来(window, view) {
  view.on("touch_down", function () {
    window.requestFocus();
    view.requestFocus();
  });
  view.on("key", function (keyCode, event) {
