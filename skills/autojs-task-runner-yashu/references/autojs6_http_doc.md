# HTTP

`http` 模块主要用于发送 HTTP 请求，获取并解析 HTTP 响应。

---

## http

### http.get

#### get(url)

**Overload 1/3**

- **参数**：
  - `url` {string} — 请求的 URL 地址（默认使用 HTTP 协议）
- **返回值**：{HttpResponse} — 请求的响应实例

```javascript
let response = http.get('www.github.com');
if (response.statusCode === 200) {
    console.log('请求成功');
} else {
    console.log('请求失败');
}
```

#### get(url, options)

**Overload 2/3**

- **参数**：
  - `url` {string} — 请求的 URL 地址（默认使用 HTTP 协议）
  - `options` {HttpRequestBuilderOptions} — 请求的构建选项
- **返回值**：{HttpResponse} — 请求的响应实例

#### get(url, options, callback)

**Overload 3/3 Async**

- **参数**：
  - `url` {string} — 请求的 URL 地址（默认使用 HTTP 协议）
  - `options` {HttpRequestBuilderOptions} — 请求的构建选项
  - `callback` {Function} — 请求的响应回调
- **返回值**：{HttpResponse} — 请求的响应实例

---

**详细说明：**

- `url` {string} — 请求的 URL 地址
- `options` {Object} — 请求选项。参见 [http.request()](#httprequest)
- `callback` {Function} — 回调函数，可选，其参数是一个 [Response](#response) 对象。如果不加回调函数，则该请求将阻塞、同步地执行

对地址 url 进行一次 HTTP GET 请求。如果没有回调函数，则在请求完成或失败时返回此次请求的响应（参见 [Response](#response)）。

**最简单的 GET 请求示例：**

```javascript
console.show();
var r = http.get("www.baidu.com");
log("code = " + r.statusCode);
log("html = " + r.body.string());
```

**采用回调形式的 GET 请求：**

```javascript
console.show();
http.get("www.baidu.com", {}, function(res, err){
    if(err){
        console.error(err);
        return;
    }
    log("code = " + res.statusCode);
    log("html = " + res.body.string());
});
```

**增加 HTTP 头部信息：**

```javascript
console.show();
var r = http.get("www.baidu.com", {
    headers: {
        'Accept-Language': 'zh-cn,zh;q=0.5',
        'User-Agent': 'Mozilla/5.0(Macintosh;IntelMacOSX10_7_0)AppleWebKit/535.11(KHTML,likeGecko)Chrome/17.0.963.56Safari/535.11'
    }
});
log("code = " + r.statusCode);
log("html = " + r.body.string());
```

**请求天气并解析 JSON 结果的例子：**

```javascript
var city = "广州";
var res = http.get("http://www.sojson.com/open/api/weather/json.shtml?city=" + city);
if(res.statusCode != 200){
    toast("请求失败: " + res.statusCode + " " + res.statusMessage);
}else{
    var weather = res.body.json();
    log(weather);
    toast(util.format("温度: %s 湿度: %s 空气质量: %s", weather.data.wendu,
        weather.data.shidu, weather.quality));
}
```

---

### http.post

#### post(url, data, options?, callback?)

- **参数**：
  - `url` {string} — 请求的 URL 地址，需要以 `"http://"` 或 `"https://"` 开头。如果 url 没有以 `"http://"` 开头，则默认为 `"http://"`
  - `data` {string | Object} — POST 数据
  - `options` {Object} — 请求选项
  - `callback` {Function} — 回调，其参数是一个 [Response](#response) 对象。如果不加回调参数，则该请求将阻塞、同步地执行

对地址 url 进行一次 HTTP POST 请求。如果没有回调函数，则在请求完成或失败时返回此次请求的响应（参见 [Response](#response)）。

其中 POST 数据可以是字符串或键值对。具体含义取决于 `options.contentType` 的值。默认为 `"application/x-www-form-urlencoded"`（表单提交），这种方式是 JQuery 的 ajax 函数的默认方式。

**模拟表单提交登录淘宝的例子：**

```javascript
var url = "https://login.taobao.com/member/login.jhtml";
var username = "你的用户名";
var password = "你的密码";
var res = http.post(url, {
    "TPL_username": username,
    "TPL_password": password
});
var html = res.body.string();
if(html.contains("页面跳转中")){
    toast("登录成功");
}else{
    toast("登录失败");
}
```

---

### http.postJson

#### postJson(url, data?, options?, callback?)

- **参数**：
  - `url` {string} — 请求的 URL 地址，需要以 `"http://"` 或 `"https://"` 开头。如果 url 没有以 `"http://"` 开头，则默认为 `"http://"`
  - `data` {Object} — POST 数据
  - `options` {Object} — 请求选项
  - `callback` {Function} — 回调，其参数是一个 [Response](#response) 对象。如果不加回调参数，则该请求将阻塞、同步地执行

以 JSON 格式向目标 Url 发起 POST 请求。如果没有回调函数，则在请求完成或失败时返回此次请求的响应（参见 [Response](#response)）。

JSON 格式指的是，将会调用 `JSON.stringify()` 把 data 对象转换为 JSON 字符串，并在 HTTP 头部信息中把 `"Content-Type"` 属性置为 `"application/json"`。这种方式是 AngularJS 的 ajax 函数的默认方式。

**调用图灵机器人接口的例子：**

```javascript
var url = "http://www.tuling123.com/openapi/api";
r = http.postJson(url, {
    key: "65458a5df537443b89b31f1c03202a80",
    info: "你好啊",
    userid: "1",
});
toastLog(r.body.string());
```

---

### http.postMultipart

#### postMultipart(url, files, options?, callback?)

- **参数**：
  - `url` {string} — 请求的 URL 地址，需要以 `"http://"` 或 `"https://"` 开头。如果 url 没有以 `"http://"` 开头，则默认为 `"http://"`
  - `files` {Object} — POST 数据
  - `options` {Object} — 请求选项
  - `callback` {Function} — 回调，其参数是一个 Response 对象。如果不加回调参数，则该请求将阻塞、同步地执行

向目标地址发起类型为 `multipart/form-data` 的请求（通常用于文件上传等）。其中 files 参数是 `{name1: value1, name2: value2, ...}` 的键值对，value 的格式可以是以下几种情况：

1. string
2. 文件类型，即 `open()` 返回的类型
3. `[fileName, filePath]`
4. `[fileName, mimeType, filePath]`

其中 1 属于非文件参数，2、3、4 为文件参数。

**格式 2 示例（最简单的文件上传）：**

```javascript
var res = http.postMultipart(url, {
    file: open("/sdcard/1.txt")
});
log(res.body.string());
```

**格式 3 示例：**

```javascript
var res = http.postMultipart(url, {
    file: ["1.txt", "/sdcard/1.txt"]
});
log(res.body.string());
```

**格式 4 示例：**

```javascript
var res = http.postMultipart(url, {
    file: ["1.txt", "text/plain", "/sdcard/1.txt"]
});
log(res.body.string());
```

**附带非文件参数的示例：**

```javascript
var res = http.postMultipart(url, {
    appId: "adcdefghijk",
    file: open("/sdcard/1.txt")
});
log(res.body.string());
```

---

### http.request

#### request(url, options?, callback?)

- **参数**：
  - `url` {string} — 请求的 URL 地址，需要以 `"http://"` 或 `"https://"` 开头。如果 url 没有以 `"http://"` 开头，则默认为 `"http://"`
  - `options` {Object} — 请求选项。参见 [http.buildRequest()](#httpbuildrequest)
  - `callback` {Function} — 回调，其参数是一个 [Response](#response) 对象。如果不加回调参数，则该请求将阻塞、同步地执行

对目标地址 url 发起一次 HTTP 请求。如果没有回调函数，则在请求完成或失败时返回此次请求的响应（参见 [Response](#response)）。

**选项 options 可以包含以下属性：**

| 属性 | 类型 | 说明 |
|------|------|------|
| `headers` | Object | 键值对形式的 HTTP 头部信息。有关 HTTP 头部信息，参见[菜鸟教程：HTTP 响应头信息](http://www.runoob.com/http/http-header-fields.html) |
| `method` | string | HTTP 请求方法。包括 `"GET"`, `"POST"`, `"PUT"`, `"DELETE"`, `"PATCH"` |
| `contentType` | string | HTTP 头部信息中的 `"Content-Type"`，表示 HTTP 请求的内容类型。例如 `"text/plain"`, `"application/json"`。更多信息参见[菜鸟教程：HTTP contentType](http://www.runoob.com/http/http-content-type.html) |
| `body` {string \| Array \| Function} | HTTP 请求的内容。可以是一个字符串，也可以是一个字节数组；或者是一个以 [BufferedSink](https://github.com/square/okio/blob/master/okio/src/main/java/okio/BufferedSink.java/) 为参数的函数 |

该函数是 `get`, `post`, `postJson` 等函数的基础函数。因此除非是 PUT, DELETE 等请求，或者需要更高定制的 HTTP 请求，否则直接使用 `get`, `post`, `postJson` 等函数会更加方便。

---

### http.buildRequest

#### buildRequest(url, options)

- **参数**：
  - `url` {string} — 请求的 URL 地址
  - `options` {Object} — 请求选项
- **返回值**：{Request}
- **说明**：构建一个 HTTP 请求对象并返回。该请求对象可以用于更灵活的 HTTP 请求控制。

---

## HttpResponse（Response）

HTTP 响应对象，表示一次 HTTP 请求的响应结果。

### 属性

| 属性 | 类型 | 说明 |
|------|------|------|
| `statusCode` | number | HTTP 响应状态码（如 200 表示成功，404 表示未找到） |
| `statusMessage` | string | HTTP 响应状态消息 |
| `headers` | Object | HTTP 响应头部信息 |
| `body` | ResponseBody | 响应体对象 |

### body 方法

#### body.string()

- **返回值**：{string}
- **说明**：以字符串形式返回响应体内容

#### body.json()

- **返回值**：{Object}
- **说明**：以 JSON 对象形式返回响应体内容（自动解析 JSON）

#### body.bytes()

- **返回值**：{byte[]}
- **说明**：以字节数组形式返回响应体内容

---

## HttpRequestBuilderOptions

HTTP 请求构建选项对象，用于配置 HTTP 请求的各项参数。

### 主要属性

| 属性 | 类型 | 说明 |
|------|------|------|
| `url` | string | 请求的 URL |
| `method` | string | 请求方法（GET / POST / PUT / DELETE 等） |
| `headers` | Object | 请求头 |
| `contentType` | string | 内容类型 |
| `body` | string \| Object \| Function | 请求体 |
| `timeout` | number | 超时时间（毫秒） |

---

## 参考链接

- [菜鸟教程：HTTP 响应头信息](http://www.runoob.com/http/http-header-fields.html)
- [菜鸟教程：HTTP contentType](http://www.runoob.com/http/http-content-type.html)
- [OkHttp BufferedSink](https://github.com/square/okio/blob/master/okio/src/main/java/okio/BufferedSink.java/)
