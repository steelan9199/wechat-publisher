...
2. **配置应用凭证**：将应用的 `App ID` 和 `App Secret` 填写到 `$SKILL_DIR/config.default.json` 文件中
3. **启用权限**：确认应用已开通且不限于 `创建及编辑新版文档` 权限， 原则： 需要什么就开通什么，避免开通不必要的权限

## 凭证读取优先级

1. **第一优先级**：用户主动提供的凭证 — 如果用户在对话中主动提供了 `appId` 和 `appSecret`，则使用用户提供的凭证
2. **第二优先级**：自动从 `$SKILL_DIR/config.default.json` 文件读取已配置的 `appId` 和 `appSecret`。如果配置文件中有可用的凭证，直接使用，**禁止向用户索要**（避免工作暂停等待用户判断）

⚠️ **AI自动操作规范**：执行前必须先运行 `Read` 工具读取 `$SKILL_DIR/config.default.json` 文件，使用其中已配置的凭证，禁止向用户索要。仅当用户主动提供新凭证时，使用用户提供的凭证。
...
- `secretKeyConfigFilePath` (string, 必填): 用于保存appId和appSecret, 以及tenant_access_token的配置文件的绝对路径(绝对路径分隔符使用正斜杆"/")，即 `$SKILL_DIR/config.default.json` 文件的绝对路径
...
- ✅ **正确示例**：`--parameter-file-path D:/skill/temp/temp-params.json`
...
1. 必须先运行 `Read` 工具读取 `$SKILL_DIR/config.default.json` 文件
2. 参数文件必须包含 **3个必填字段**：`appId`、`appSecret`、`secretKeyConfigFilePath`，缺一不可
3. 直接从读取结果中复制 `appId` 和 `appSecret` 到参数文件
4. `secretKeyConfigFilePath` 必须填写 `$SKILL_DIR/config.default.json` 文件的绝对路径（使用正斜杠分隔符）
