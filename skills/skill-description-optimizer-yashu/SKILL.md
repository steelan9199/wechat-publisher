...
description: 本技能专门优化技能的 description 元属性。激活条件：用户消息须包含以下关键词之一:`优化
  description`、`优化技能描述`、`重写 description`、`改 description`、`诊断 description
  问题`、`修复技能不激活`。
...
AI 在毫秒级做出路由判断，description 应控制在 **≤150 字**，关键信息前置。

### 5. 长度硬上限：150 字（Hard Limit）

**description 的总长度必须在 150 字以内，这是一条不可协商的硬性规则。**

- **计数口径**：按字符数统计（中文字、英文字母、数字、标点、空格均计入），即 `len(description)` ≤ 150。
- **超限处理**：若优化后的 description 超过 150 字，必须继续删减，直到达标。删减优先级：内容构成 > 价值宣传 > 重复表述 > 次要关键词。
- **不得超限的理由**：超长 description 会稀释关键信号、抬高注意力分配的噪声占比，还会挤占其他技能的 description 空间。长度本身就是路由质量的一部分。
- **不得以"信息完整"为由超限**：信息完整性不是目标，路由准确性才是。宁可少列 1-2 个关键词，也不得突破 150 字。

```
正确示例（136 字）：本技能专门优化技能的 description 元属性。激活条件：用户消息须包含以下关键词之一:`优化 description`、`优化技能描述`、`重写 description`、`改 description`、`诊断 description 问题`、`修复技能不激活`。
错误示例：在功能概述、激活条件之外，再补充"本技能还包含诊断清单、重写模板、冲突检测流程、多轮校验机制……"（内容构成堆砌，必然超限）
```
...
**输出前必须做长度校验**：统计新 description 的字符数，若 > 150 字，按"5. 长度硬上限"的优先级继续删减并重新计数，直到 ≤150 字方可进入步骤 4。在结果中明确给出字数（如"共 136 字，符合 ≤150 字上限"）。
...
- **150 字硬上限**：任何优化结果都必须在 150 字以内，超限即视为未完成，必须继续删减。
