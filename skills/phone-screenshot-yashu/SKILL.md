...
当需要向用户提及 AutoJS（如提示安装、运行脚本时），若用户可能不了解它，应简短介绍并附上官网链接。

- **AutoJS 是什么**：一款安卓（Android）设备的自动化工具，支持用 JavaScript 编写脚本，模拟点击、滑动、截屏等手机操作，无需 root。
- **官网**：https://docs.autojs6.com/#/
- **适用场景**：本技能通过它在手机端运行一个连接脚本，接收电脑指令完成截屏并回传图片。

**介绍示例**（供 AI 参考语气）：

> AutoJS 是一款安卓自动化工具，可以用 JavaScript 脚本控制手机完成截屏、点击等操作。本技能需要它在手机端配合运行。
> 官网文档：https://docs.autojs6.com/#/
...
| 7    | 检查手机连接      | 从步骤 3/6 的健康检查返回中读取 `phone` 字段。若为 `disconnected`，按【手机未连接提示】向用户介绍 AutoJS 并引导在手机上运行 `autojs-client.js` 脚本，任务结束。                                                                                      |
| 8    | 触发截图          | 运行 `node "$SKILL_DIR/scripts/trigger.js"`。脚本输出图片的绝对路径。                                                                                                                                                                                |
| 9    | 打开图片          | 拿到图片路径后，必须调用 `image-pixel-viewer-yashu` 技能打开该图片。若用户电脑未安装该技能，先运行 `npx skills add https://github.com/steelan9199/wechat-publisher/tree/main/skills/image-pixel-viewer-yashu` 安装，安装完成后再调用该技能打开图片。 |
...
当健康检查返回 `phone: "disconnected"` 时，按以下要点回复用户：

1. **简短介绍 AutoJS**：参考【AutoJS 简介】中的介绍示例，说明它是什么、本技能为何需要它。
2. **附上官网**：给出链接 https://docs.autojs6.com/#/ ，便于用户下载安装与查阅文档。
3. **引导运行脚本**：告知用户在手机端 AutoJS 中运行 `$SKILL_DIR/scripts/autojs-client.js`，并需在其中配置电脑的局域网 IP。
4. 前置条件须满足：手机与电脑在同一局域网、AutoJS 已开启截图权限和无障碍服务。
5. **提醒截图权限弹框的同意按钮**：脚本中有一段被注释的代码（约第 18-21 行），用于自动点击截图权限弹框的"同意"按钮。不同手机的按钮文案可能不同（如"立即开始"、"允许"、"同意"等），如果用户希望自动点击同意按钮，需要根据自己手机实际的按钮文案修改 `text("立即开始")` 中的文字，并取消该段代码的注释；若文案不匹配会导致自动点击失效。对代码不熟悉的用户也可保持注释状态，每次手动点击同意即可。
...
- **手机未连接**：按【手机未连接提示】向用户介绍 AutoJS 并引导运行手机端脚本
- **服务器未启动**：提示正在启动服务器
...
| 手机未连接     | 健康检查返回 `phone: "disconnected"` | 按【手机未连接提示】介绍 AutoJS 并附官网链接，引导运行手机端脚本 |
