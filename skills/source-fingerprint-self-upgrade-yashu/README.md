# 源码指纹自升级（Source Fingerprint Self-Upgrade）

## 这是什么

"源码指纹自升级"是一种让程序在启动时**自己判断该不该启动、要不要替换掉已经在跑的旧程序**的做法。

核心思路：不靠人手动输出版本号，而是**直接对程序自身的源码算一个哈希指纹**。只要源码有任何改动，指纹就变；源码没动，指纹就不变。
...
指纹模块 `demo/fingerprint.js` 的规则如下，务必照此实现：
...
**关键：非 JS 脚本（如 `demo/scripts/run.sh`）是程序的一部分，但按规则不计入指纹。** 因为改个部署脚本不该触发"代码升级重启"。如果你希望某些非 JS 文件也计入，可以在规则里放开——但默认只认 `.js`。
...
│   │       ├── helper.js        # 嵌套子模块
│   │       └── deeper/
│   │           └── nested.js    # 更深一层嵌套，验证递归扫描
│   └── scripts/
│       ├── run.sh       # 非 JS 脚本（不计入指纹）
│       └── setup.py     # 非 JS 脚本（不计入指纹）
└── test/
    └── test.js          # 最简单的测试
```
